...
class FilterScheduler(driver.Scheduler):
	...
	print "\t\tFilter Props : ", filter_properties
        ...
        # First read all the limits in nova.limits file if limits are enabled
	print "Instance props : ", request_spec['instance_properties']
	print "Metadata : ", request_spec['instance_properties']['metadata']
        metadata = request_spec['instance_properties']['metadata']
        limits_enabled = False;
        try:
            strval = metadata['limits']
            if strval in ['True', 'true']:
                limits_enabled = True
            else:
                limits_enabled = False
        except KeyError:
        ...
        limitsdict = {} 
        if limits_enabled:
            limits_file = open("/opt/stack/nova/nova.limits", "r")
            for line in limits_file:
                keyval = line.split(":")
                limitsdict[keyval[0]] = int(keyval[1])
            limits_file.close()
        ...
        for num in xrange(num_instances):
            ...
            print "\t\tWeighed hosts : ", weighed_hosts
            print "\t\tLimits : ", limits_enabled, limitsdict
            if limits_enabled:
                ...
                # Now we have three objects : 
                # 1. a dictionary limitsdict of (host_name:user_limit)
                # 2. a dictionary current_state_dict of (hostname:number_of_instances_running)
                # 3. a list weighed_hosts of WeighedHost objects
                # We now have to remove the WeighedHost objects from list in (3) that do not conform to limits in (1)
                to_remove = []
                for whost in weighed_hosts:
                    print "\t\tWHOST : ", whost
                    userlimit = 0
                    currload = 0
                    try:
                        userlimit = limitsdict[whost.obj.host]
                        print "\t\tUserlimit : ", userlimit
                    except KeyError:
                        userlimit = -1

                    try:
                        currload = current_state_dict[whost.obj.host]
                        print "\t\tCurrent Load : ", currload;
                    except KeyError:
                        currload = -1

                    if (userlimit != -1) and (userlimit <= currload):
                        # This host has reached its limit. Queue it for removing from weighed_hosts
                        print "\t\tWe want to remove : ", whost
                        to_remove.append(whost)

                print "\t\tObjects to remove : ", to_remove
                for whost in to_remove:
                    print "\t\tRemoving ", whost
                    weighed_hosts.remove(whost)
